...
split_end = 0.01
split_end_line = int(len(comments_data) * split_end)
...
train_comments, train_labels = list(comments_data[:split_line][0]), list(comments_data[: split_line][1])
test_comments, test_labels = list(comments_data[split_line:split_end_line][0]), list(
    comments_data[split_line:split_end_line][1])

# 查看训练集和测试集数量
print('\n训练集数量：', len(train_comments), len(train_labels), '\n测试集数量：', len(test_comments), len(test_labels))
...
class BERTClassifier(nn.Module):
    ...
    def __init__(self, output_dim, pretrained_name='./model/bert-base-chinese/'):
